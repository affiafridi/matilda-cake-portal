...
const WA_NAV = [
  { href: "/customers",   label: "Customers",         icon: IcCustomers },
  { href: "/wa/templates",label: "Send Campaign",      icon: IcSend },
  { href: "/wa/campaigns",label: "Campaign History",   icon: IcHistory },
  { href: "/wa/manage",   label: "Manage Templates",   icon: IcTemplate },
  { href: "/wa/settings", label: "Settings",           icon: IcSettings },
];
...
export default function AppShell({
  ...
  return (
    <div className="flex min-h-screen bg-canvas">
      ...
      {/* ── Sidebar ── */}
      <aside className={[
        "fixed inset-y-0 left-0 z-50 flex w-[260px] flex-col bg-white",
        ...
        <nav className="scrollbar-thin flex-1 overflow-y-auto px-3 py-3 space-y-3">
          ...
          {/* PORTAL section */}
          <div>
            <p className="mb-1 px-3 text-[10px] font-bold uppercase tracking-[0.12em] text-[#7a5c4d]/60">Menu</p>
                ...
                return (
                  <Link key={item.href} href={item.href}
                    className={[
                      ...
                      active
                        ? "bg-[#6b2e1a] text-white"
                        : "text-[#7a5c4d] hover:bg-[#f5e6d3]/60 hover:text-[#2b1a12]",
                    ].join(" ")}>
                    <span className={["flex h-7 w-7 shrink-0 items-center justify-center rounded-lg transition-colors",
                      active ? "bg-white/15" : "bg-[#f5e6d3] group-hover:bg-[#f5e6d3]",
                    ].join(" ")}>
                      <Icon className={["h-4 w-4", active ? "text-white" : "text-[#6b2e1a]"].join(" ")} />
                    </span>
                    {item.label}
                  </Link>
                );
                    ...
                    <Link key={item.href} href={item.href}
                      className={[
                        ...
                        active
                          ...
                          : "text-[#7a5c4d] hover:bg-[#e8f5e9]/60 hover:text-[#075E54]",
                      ].join(" ")}>
                      <span className={["flex h-7 w-7 shrink-0 items-center justify-center rounded-lg transition-colors",
                    ...
                    <Link key={item.href} href={item.href}
                      className={[
                        ...
                        active
                          ...
                          : "text-[#7a5c4d] hover:bg-[#f3eeff]/60 hover:text-[#7f54b3]",
                      ].join(" ")}>
                      <span className={["flex h-7 w-7 shrink-0 items-center justify-center rounded-lg transition-colors",
                    ...
                    <Link key={item.href} href={item.href}
                      className={[
                        ...
                        active
                          ...
                          : "text-[#7a5c4d] hover:bg-[#f5f5f4]/80 hover:text-[#1c1917]",
                      ].join(" ")}>
                      <span className={["flex h-7 w-7 shrink-0 items-center justify-center rounded-lg transition-colors",
          ...
          {/* GENERAL section */}
          <div>
            <p className="mb-1 px-3 text-[10px] font-bold uppercase tracking-[0.12em] text-[#7a5c4d]/60">General</p>
            <div className="space-y-0.5">
              <button type="button" onClick={signOut} disabled={signing}
                className="group flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm font-medium text-[#7a5c4d] transition-all hover:bg-red-50 hover:text-red-600 disabled:opacity-50">
                <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-[#f5e6d3] group-hover:bg-red-100">
                  <IcLogout className="h-4 w-4 text-[#6b2e1a] group-hover:text-red-500" />
                </span>
        ...
        {/* User card */}
        <div className="shrink-0 border-t border-[#f0ebe4] p-3">
          <div className="flex items-center gap-3 rounded-xl bg-[#faf7f2] px-3 py-2.5">
            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-[#6b2e1a] text-xs font-bold text-white">
              {initials || "U"}
            </div>
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm font-semibold text-[#2b1a12]">{user.name}</p>
              <p className="truncate text-[11px] text-[#7a5c4d]">{ROLE_LABEL[user.role]}</p>
            </div>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        {/* Top header */}
        <header className="sticky top-0 z-30 flex h-[64px] shrink-0 items-center justify-between border-b border-[#e8ddd4] bg-white px-6 lg:px-8">
            ...
            <div className="hidden items-center gap-2 sm:flex">
              {meta.parent ? (
                <>
                  ...
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" className="h-3.5 w-3.5 text-[#c08a5b]" aria-hidden="true">
          ...
          <div className="flex items-center gap-2">
            ...
            <button type="button"
              ...
              <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-[#6b2e1a] text-[11px] font-bold text-white">
                {initials || "U"}
              </div>
              <div className="hidden text-left sm:block">
                <p className="text-xs font-semibold leading-tight text-[#2b1a12]">{user.name.split(" ")[0]}</p>
                <p className="text-[10px] leading-tight text-[#c08a5b]">{ROLE_LABEL[user.role]}</p>
              </div>
