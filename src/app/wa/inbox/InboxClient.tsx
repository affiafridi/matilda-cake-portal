    ...
    <div className="flex overflow-hidden bg-[#f7f8fa]" style={{ height: "calc(100vh - 64px)" }}>
      ...
      <div className="flex w-[300px] shrink-0 flex-col border-r border-gray-200 bg-white">
        ...
        <div className="overflow-y-auto flex-shrink-0">
          {/* CONVERSATIONS button group */}
          <div className="px-3 pt-3 pb-0">
            <p className="mb-1.5 text-[9px] font-semibold uppercase tracking-widest text-gray-400">Conversations</p>
            <div className="flex rounded-md border border-gray-200 overflow-hidden">
              {([
                { key: "unassigned", label: "Unassigned" },
                { key: "mine",       label: "Assigned" },
                { key: "all",        label: "All Active" },
              ] as { key: typeof view; label: string }[]).map(({ key, label }, i, arr) => (
                <button key={key} onClick={() => setView(key)}
                  className={["flex flex-1 items-center justify-center gap-1 py-1.5 text-[12px] font-medium transition-colors",
                    i < arr.length - 1 ? "border-r border-gray-200" : "",
                    view === key ? "bg-brand text-white" : "bg-white text-gray-600 hover:bg-gray-50",
                  ].join(" ")}>
                  <span className="truncate">{label}</span>
                  {(counts[key as keyof typeof counts] ?? 0) > 0 && (
                    <span className={["rounded-full px-1.5 py-px text-[9px] font-bold leading-none",
                      view === key ? "bg-white/25 text-white" : "bg-gray-100 text-gray-500",
                    ].join(" ")}>
          ...
          <div className="px-3 pt-3 pb-0">
            ...
            <div className="flex rounded-md border border-gray-200 overflow-hidden">
              ...
              ] as { key: typeof view; label: string; dot: string }[]).map(({ key, label, dot }, i, arr) => (
                <button key={key} onClick={() => setView(key)}
                  className={["flex flex-1 items-center justify-center gap-1.5 py-1.5 text-[12px] font-medium transition-colors",
                    i < arr.length - 1 ? "border-r border-gray-200" : "",
                    view === key ? "bg-brand text-white" : "bg-white text-gray-600 hover:bg-gray-50",
                  ].join(" ")}>
                  <span className={["h-1.5 w-1.5 shrink-0 rounded-full", view === key ? "bg-white/70" : dot].join(" ")} />
                  <span className="truncate">{label}</span>
                  {(counts[key as keyof typeof counts] ?? 0) > 0 && (
                    <span className={["rounded-full px-1.5 py-px text-[9px] font-bold leading-none",
                      view === key ? "bg-white/25 text-white" : "bg-gray-100 text-gray-500",
        ...
        {!selectedId ? (
          ...
          <>
            ...
            <div className="relative flex-1 flex flex-col min-h-0">
              ...
              <div ref={scrollRef} className="relative flex-1 overflow-y-auto px-4 py-4 space-y-1"
                style={isIgSelected
                  ? { background: "#fafafa" }
                ...
                {/* Scroll-to-bottom — sticky to bottom of chat feed viewport */}
                {showScrollBtn && (
