...
export default function CustomersPage({ isSuperAdmin = false }: { isSuperAdmin?: boolean }) {
  ...
  async function handleDelete() {
    ...
    if (!window.confirm(`Delete ${count} customer${count !== 1 ? "s" : ""} and all their conversations? This cannot be undone.`)) return;
    setDeleting(true);
    try {
      await Promise.all([...selected].map((waId) =>
        fetch(`/api/admin/customers/${encodeURIComponent(waId)}`, { method: "DELETE" })
      ));
      setCustomers((p) => p.filter((c) => !selected.has(c.wa_id)));
      setSelected(new Set());
    } finally {
      setDeleting(false);
    }
