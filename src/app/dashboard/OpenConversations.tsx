    ...
    <div className="rounded-2xl border border-rule bg-white p-5">
      <div className="mb-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <p className="text-sm font-bold text-ink">Open Conversations</p>
          {pulse && (
            <span className="flex h-2 w-2 rounded-full bg-[#25D366] animate-ping" />
          )}
        </div>
        <Link href="/wa/inbox" className="text-xs font-semibold text-caramel hover:text-brand transition">
          Go to inbox →
        </Link>
      </div>
      ...
      {convs.length === 0 ? (
        <div className="py-8 text-center">
          <p className="text-xs text-ink-muted">No open conversations.</p>
        </div>
      ) : (
        <ul className="divide-y divide-rule">
          {convs.map((conv) => (
            <li key={conv.id}>
              <Link href="/wa/inbox" className="flex items-start gap-3 py-2.5 -mx-1 px-1 rounded-xl transition hover:bg-canvas">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between gap-1">
                    <p className="truncate text-xs font-semibold text-ink">{conv.customerName}</p>
                    <span className="shrink-0 text-[10px] text-ink-muted whitespace-nowrap">{timeAgo(conv.lastMessageAt)}</span>
                  </div>
                  <p className="truncate text-[11px] text-ink-muted">{conv.lastMessageBody ?? "Media message"}</p>
                </div>
                {conv.unreadCount > 0 && (
